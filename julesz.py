...
def get_histogram(lib, image, synthesized, filters, width, height, im_width, im_height, num_bins, min_response, max_response, is_syn):
    ...
    return response, min_response[0], max_response[0]
...
def main():
    ...
    bin_num = 15
    threshold = 200
    weights = np.array([8, 7, 6, 5, 4, 3, 2, 1, 2, 3, 4, 5, 6, 7, 8])
    ...
    while len(chosen_idx) < use_num:
        ...
        if np.max(err) < threshold:
            break
        idx = np.argmax(err)
        chosen_idx.append(unchosen_idx[idx])
        unchosen_idx.remove(unchosen_idx[idx])

        # gibbs sampling
        synthesized, final_res = gibbs(lib, image, synthesized, filters[chosen_idx], width[chosen_idx], height[chosen_idx], im_w, im_h, bin_num, weights, max_intensity)
        ...
        for i in chosen_idx:
            ...
            syn_res, _, _ = get_histogram(lib, synthesized, image, np.array([filters[i]]), np.array([width[i]]), np.array([height[i]]), im_w, im_h, bin_num, min_res, max_res, 1)
            err.append(np.sum(abs(im_res[0] - syn_res[0])))
            weighted_err.append(np.sum(abs(im_res[0] - syn_res[0]) * weights))
